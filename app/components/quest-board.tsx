...
import { questIcon, questTier, type Bounty, type FamilyBankState, type KidProfile } from "@/lib/schema";
    ...
    <section className="space-y-4 rounded-2xl border-2 border-dashed border-amber-500/40 bg-amber-500/5 p-4">
      ...
      {open.length === 0 ? (
        ...
        <div className="space-y-2">
          {open.map((bounty) => (
            <QuestCard key={bounty.id} bounty={bounty} onClaim={() => onMutate((state) => claimBounty(state, bounty.id, kid.id))} />
          ))}
        </div>
      )}
